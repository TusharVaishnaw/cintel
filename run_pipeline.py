...
Usage:
    ...
    python run_pipeline.py "Apple" --year 2025 --cik 320193 --ticker AAPL
        (skips the interactive company picker entirely — used by the
        webapp, which resolves the pick itself via fetch.search_company()
        before launching this script)
    (without --cik/--ticker, company search/confirm happens interactively
    inside fetch.py, same as always)
...
# Fixed, known stage order — every run hits these prints in exactly this
# sequence (evaluate/fetch_photos/build_ppt are the only ones that can be
# skipped via flags or fail without aborting). A driving UI (webapp) counts
# these markers instead of parsing the human-readable "=== RUNNING ===" text,
# which changes wording/args run to run (10-K vs 10-Q fetch, ticker/year).
STAGE_SEQUENCE = [
...
def stage_marker(stage_id):
    print(f"\n### STAGE {stage_id} ###", flush=True)
...
def main():
    ...
    parser.add_argument("--cik", type=int, default=None, help="skip interactive company picker (webapp pre-resolves this)")
    parser.add_argument("--ticker", default=None, help="required together with --cik")
    ...
    if not args.no_ppt:
        ...
        stage_marker("extract_slide_data")
        run_step([sys.executable, "extract_slide_data.py", final_json, slide_data])
        stage_marker("fetch_photos")
        if not args.no_photos:
            run_step_optional([sys.executable, "fetch_photos.py", slide_data])
        stage_marker("build_ppt")
        run_step_optional([sys.executable, "build_ppt.py", slide_data, deck])

        print(f"\n=== PPT — {deck} ===" if os.path.exists(deck) else "\n=== PPT build skipped/failed — template.json still saved above ===")
        print(f"\n### DONE ticker={ticker} year={year} deck={deck if os.path.exists(deck) else ''} ###", flush=True)
    else:
        print(f"\n### DONE ticker={ticker} year={year} deck= ###", flush=True)

    # advisory only, never blocks anything, nothing downstream reads it —
    # extract_slide_data.py reads template.json and writes a SEPARATE
    # slide_data.json; it never mutates final_json, so evidence/source are
    # still intact here even though this runs after the PPT stages. Moved
    # to last so the deck (what the person actually wants) finishes before
    # this ~10+ min sequential judge-model pass even starts.
    stage_marker("evaluate")
    run_step_optional([sys.executable, "evaluate.py", ticker, year])
