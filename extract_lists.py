    ...
    for row in rows:
        ...
        if any(title in designation for title in OFFICER_TITLES):
            ...
            kept.append({
                ...
                "_evidence": {
                    "name": evi.get("name"), "designation": evi.get("designation"),
                    "brief": evi.get("brief"),
                },
            })
    return kept
...
def build_segment_rows(xbrl_segs, llm_rows, object_fields):
    ...
    for seg_name, data in filtered_segs.items():
        ...
        evi = {
            "segment_name": f"XBRL StatementBusinessSegmentsAxis member: {seg_name}",
            "revenue": f"{SEGMENT_REVENUE_TAG}: {row['revenue']} (segment, current FY)" if row["revenue"] is not None else None,
            "operating_income": f"{SEGMENT_OPINCOME_TAG}: {row['operating_income']} (segment, current FY)" if row["operating_income"] is not None else None,
        }
        if match:
            row["description"] = match.get("description")
            row["key_services"] = match.get("key_services")
            match_evi = match.get("_evidence", {})
            evi["description"] = match_evi.get("description")
            evi["key_services"] = match_evi.get("key_services")
        row["_evidence"] = evi
        rows.append(row)
    return rows
...
Return ONLY a JSON array. Each array element is an object; for EACH key in
[{fields_str}], the value is itself an object {{"value": <fact or null>,
"evidence": <verbatim supporting sentence from the section text, or null>}}.
evidence must be an EXACT quote from the section text, not a paraphrase.
Use null value for any key you cannot find. If nothing relevant found, return [].
Do not invent data.
...
def _unwrap_row(row):
    """Row from extract_list_section: {field: {value, evidence}}. Also
    handles pre-existing plain-value rows (XBRL-built rows, filter_officers
    output) which have no evidence — returns (plain_values_dict, evidence_dict)."""
    if not isinstance(row, dict):
        return {}, {}
    values, evidence = {}, {}
    for k, v in row.items():
        if isinstance(v, dict) and "value" in v and "evidence" in v:
            values[k] = v.get("value")
            evidence[k] = v.get("evidence")
        else:
            values[k] = v
            evidence[k] = None
...
def xbrl_acquisition_rows(raw_dir, source_url, as_of, object_fields):
    ...
    for period_facts, year_label in (
        ...
        for tag in ACQUISITION_XBRL_TAGS:
            ...
            if val and val != 0:
                ...
                row["_evidence"] = {"acquisition_value": f"{tag}: {val} (XBRL fact)"}
                rows.append(row)
    ...
    for d in xbrl_dimensioned_acquisition_facts(raw_dir):
        ...
        row["_xbrl_tag"] = d["tag"]
        row["_evidence"] = {"acquisition_value": f"{d['tag']}: {d['value']} (XBRL dimensioned fact, label={d['label']})"}
        rows.append(row)
    ...
    for d in xbrl_named_deal_facts(raw_dir):
        row = {f: None for f in object_fields}
        row["acquisition_value"] = d["value"]
        row["_evidence"] = {"acquisition_value": f"{d['tag']}: {d['value']} (XBRL fact, close={d['date']}, label={d['label']})"}
        if "year" in row:
            row["year"] = d["year"]
...
def wrap_items(raw_items, object_fields, source_url, as_of):
    """Wrap each plain dict into value/source/as_of/evidence leaf format
    matching template. evidence comes from the row's '_evidence' side-dict
    (never a template field itself, dropped here by only reading object_fields)."""
    wrapped = []
    for item in raw_items:
        obj = {}
        evi_map = item.get("_evidence", {}) if isinstance(item, dict) else {}
        for field in object_fields:
            val = item.get(field) if isinstance(item, dict) else None
            obj[field] = {
                "value": val, "source": source_url, "as_of": as_of,
                "evidence": evi_map.get(field),
            }
        wrapped.append(obj)
    return wrapped
...
def main():
    ...
    for field_path, (item_name, object_fields) in LIST_FIELDS.items():
        ...
        if not chunk:
            ...
            continue
        raw_items_llm = extract_list_section(item_name, chunk, object_fields, list_name)
        raw_items = []
        for row in raw_items_llm:
            values, evidence = _unwrap_row(row)
        ...
        if field_path == "from_annual_report.acquisitions.acquisitions":
            ...
            if item8_chunk:
                item8_rows_raw = extract_list_section("Item 8 (Business Combinations note)", item8_chunk, object_fields, "acquisitions")
                item8_rows = []
                for row in item8_rows_raw:
                    values, evidence = _unwrap_row(row)
                    values["_evidence"] = evidence
                    item8_rows.append(values)
