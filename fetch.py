...
def fetch_10k(cik: int, ticker: str, year: int | None, form: str = "10-K"):
    ...
    if form == "10-K":
        # Confirmed bug (PAPL): company.get_filings(form="10-K") also
        # returns 10-K/A amendments — EDGAR groups them under the same
        # form family. .latest() then grabbed a 10-K/A that only restated
        # internal-controls language + signatures (zero Item 1/1A/7
        # content, verified by inspecting the raw filing.html directly),
        # so extract_annual_report.py had nothing to extract from and
        # came back 1/107 filled — not an extraction bug, a wrong-filing
        # bug. Amendments almost never carry the full business/MD&A
        # sections real extraction needs, so exclude them and always
        # prefer the original 10-K.
        filings = [f for f in filings if getattr(f, "form", "10-K") == "10-K"]
        if not filings:
            raise SystemExit(
                f"No original 10-K found for CIK {cik} (only amendments/"
                f"other form variants) — nothing to extract from."
            )
    ...
    if form == "10-Q":
        ...
        filing = filings.latest() if hasattr(filings, "latest") else \
            max(filings, key=lambda f: f.filing_date)
