...
def set_leaf(obj, dotted, value, source, as_of, evidence=None):
    parts = dotted.split(".")
    node = obj
    try:
        for key in parts[:-1]:
...
def load_xbrl_facts(path):
    ...
    def collect(duration_end, instant_date):
        ...
        for r in records:
            ...
            if val is None:
                val = r.get("value")
            # Confirmed (PAPL, capital_expenditure): numeric_value can be
            # None while "value" is an empty string ("") rather than a real
            # figure — that's not a genuine zero, it's XBRL's own way of
            # saying "no numeric fact here" for this tag. Treating "" as a
            # real value wrote value="" into the template with evidence
            # text implying a real XBRL number was found — wrong on both
            # counts. Filter it out here so this field falls through to
            # the capex-proxy fallback below (or stays correctly null)
            # instead of masquerading as filled.
            if val == "":
                val = None
            if val is not None:
                facts[tag] = val
...
def _xbrl_evidence(tag, value, period_label):
    """No prose to quote for a tagged number — evidence is the matched
    fact's own record context: concept + reported value + period."""
    return f"{tag}: {value} ({period_label}, XBRL fact)"
...
def fill_from_xbrl(template, current_facts, prior_facts, second_prior_facts, source_url, as_of):
    for field_path, tag in XBRL_MAP.items():
        ...
        if value is not None:
            set_leaf(template, field_path, value, source_url, as_of,
                      evidence=_xbrl_evidence(tag, value, "current FY"))
        ...
        if field_path.endswith(".current_year"):
            ...
            if prev_value is not None:
                set_leaf(template, prev_path, prev_value, source_url, as_of,
                          evidence=_xbrl_evidence(tag, prev_value, "prior FY"))
            ...
            if two_prior_value is not None:
                set_leaf(template, two_prior_path, two_prior_value, source_url, as_of,
                          evidence=_xbrl_evidence(tag, two_prior_value, "two years prior FY"))
    ...
    if capex_leaf.get("value") is None:
        ...
        if ppe_cur is not None and ppe_prior is not None:
            ...
            if "evidence" in capex_leaf:
                capex_leaf["evidence"] = (
                    f"PropertyPlantAndEquipmentGross: {ppe_cur} (current FY) - {ppe_prior} (prior FY), XBRL facts"
                )
...
class GeminiQuotaExhausted(RuntimeError):
    """Free-tier daily/window quota hit and the API's own retry-after wait
    didn't clear it. No amount of local backoff guessing fixes this —
    every subsequent call will fail identically, so the caller must STOP,
    not keep retrying leaf-by-leaf / field-by-field for minutes."""
    pass
    ...
    for attempt in range(retries + 1):
        try:
            ...
            if resp.status_code == 429:
                ...
                api_wait = float(m.group(1)) + 1 if m else None
                if err.get("status") == "RESOURCE_EXHAUSTED":
                    # the API tells us EXACTLY how long its window needs —
                    # that number is trustworthy, our own guessed backoff
                    # is not. Respect it once; if it's STILL exhausted
                    # after that, this is a real quota cap (e.g. free-tier
                    # 500/day), not a burst — stop immediately instead of
                    # retrying blind for minutes.
                    if api_wait is not None and api_wait <= 60 and attempt == 0:
                        print(f"[warn] gemini quota window full, waiting {api_wait:.0f}s (API-reported wait, not a guess)")
                        time.sleep(api_wait)
                        continue
                    print(f"[fatal] gemini quota exhausted, not retrying further: {msg[:300]}")
                    raise GeminiQuotaExhausted(msg)
                # a plain 429 (not the exhausted-quota case) — short retry
                print(f"[warn] gemini 429: {msg[:200]}")
                if attempt < retries:
                    time.sleep(5)
                    continue
                return "{}"
            ...
            if resp.status_code == 503:
                # capacity/overload, not our bug — backs off harder than a
                # normal error and doesn't count against the caller's
                # retry budget the same way as a real failure.
                wait = 10 * (attempt + 1)
                print(f"[warn] gemini 503 (overloaded), backing off {wait}s ({attempt + 1}/{retries + 1})")
                if attempt < retries:
                    time.sleep(wait)
                    continue
                print("[warn] giving up on this call (persistent 503)")
                return "{}"
            ...
            raise  # never swallow this — every caller up the stack must stop
...
keys = field path. Each value is an object: {{"value": <extracted fact, or
null if not found>, "evidence": <verbatim supporting sentence/passage from
the section text below, or null if value is null>}}. evidence must be an
EXACT quote from the section text, not a paraphrase. Do not invent data.

CRITICAL: evidence is not just "a real quote from this section" — it must
be the SPECIFIC sentence(s) that directly state or entail this exact
value. A quote that is merely nearby, about a related topic, or true in
general but not specific to this value is NOT valid evidence — if you
cannot find a passage that directly supports the value, set both value
AND evidence to null rather than attaching an unrelated true quote.
...
def _split_value_evidence(entry):
    """LLM now returns {"value":..,"evidence":..} per field. Older/malformed
    responses may still return a bare value — treat that as value, no
    evidence, instead of crashing."""
    if isinstance(entry, dict) and "value" in entry:
        return entry.get("value"), entry.get("evidence")
    return entry, None
    ...
    for item_name, field_paths in grouped.items():
        ...
        if leaf_fields:
            for i in range(0, len(leaf_fields), 12):
                ...
                for field_path, entry in result.items():
                    if field_path not in batch:
                        print(f"[warn] LLM returned unknown field '{field_path}', ignoring")
                        continue
                    value, evidence = _split_value_evidence(entry)
                    if value is not None:
                        set_leaf(template, field_path, value, source_url, as_of, evidence=evidence)
...
def fill_combined_search(template, items_text, source_url, as_of, field_prefixes, label):
    ...
    for i in range(0, len(leaf_fields), 12):
        ...
        has_any = lambda r: any(_split_value_evidence(v)[0] is not None for v in r.values())
        if not has_any(result):
