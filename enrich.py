...
def leaf(value, source, as_of=None, evidence=None):
    """evidence = the raw fetched fragment that supports value (yfinance
    field, RSS/NewsAPI snippet, Wikipedia extract, etc) — never invented,
    None when there's genuinely nothing to quote."""
    return {"value": value, "source": source, "as_of": as_of or now_iso(), "evidence": evidence}
...
def fill_investor_information(template, ticker):
    ...
    if info:
        ...
        inv["market_cap"] = leaf(info.get("marketCap"), src, evidence=f"marketCap: {info.get('marketCap')}")
        price = info.get("currentPrice") or info.get("regularMarketPrice")
        inv["share_price"] = leaf(price, src, evidence=f"currentPrice/regularMarketPrice: {price}")
        inv["shares_outstanding"] = leaf(info.get("sharesOutstanding"), src, evidence=f"sharesOutstanding: {info.get('sharesOutstanding')}")
        inv["stock_exchange"] = leaf(info.get("exchange"), src, evidence=f"exchange: {info.get('exchange')}")
        inv["ticker"] = leaf(ticker, src, evidence=f"queried ticker: {ticker}")
        print(f"  investor_information: filled from {src_tag}")
        return
    stooq_data, stooq_url = _stooq_fallback(ticker)
    if stooq_data:
        inv["share_price"] = leaf(stooq_data["currentPrice"], stooq_url, evidence=f"stooq CSV Close: {stooq_data['currentPrice']}")
        inv["ticker"] = leaf(ticker, stooq_url, evidence=f"queried ticker: {ticker}")
...
def fill_latest_news(template, company_name, ticker=None):
    ...
    for r in results:
        ...
        if not _is_relevant_news(title, summary, r["url"], aliases):
            continue
        # Each field's evidence must literally support THAT field, not a
        # shared "snippet" string — confirmed cause of every 0.00 flag on
        # date/title in DeepEval runs (MSFT, PAPL):
        #  - date: RSS/API fetch timestamp is metadata about when THIS
        #    pipeline pulled the article, never actual text inside the
        #    summary/title — no snippet can ever "support" it, so it
        #    can't be faithfulness-checked at all. Leave evidence=None
        #    (same as XBRL leaves with nothing to quote) instead of
        #    pointing at unrelated text guaranteed to fail the check.
        #  - title: previously got the SUMMARY as evidence whenever a
        #    summary existed, so the judge compared the title against
        #    text that never contained it verbatim. Evidence for title
        #    must be the title itself.
        rows.append({
            "date": leaf(r.get("date"), r["url"], evidence=None),
            "title": leaf(title, r["url"], evidence=title),
            "summary": leaf(summary, r["url"], evidence=summary),
            "category": leaf(categorize_news(title, summary), r["url"], evidence=summary or title),
            "url": leaf(r["url"], r["url"], evidence=None),
        })
    if rows:
...
def fill_awards(template, company_name, ticker=None):
    ...
    for r in deduped:
        ...
        if not _is_award_subject(title, aliases):
            continue
        # Same fix as fill_latest_news: date has no text form to verify
        # against (fetch metadata, not filing content), and award's
        # evidence must be the award/title text itself, not the summary.
        rows.append({
            "date": leaf(r.get("date"), r["url"], evidence=None),
            "award": leaf(title, r["url"], evidence=title),
            "brief": leaf(r.get("summary"), r["url"], evidence=r.get("summary")),
        })
    if rows:
...
def fill_vision_from_wikidata(template, wikidata_qid):
    ...
    try:
        ...
        if not vision_leaf.get("value"):
            ...
            if "evidence" in vision_leaf:
                vision_leaf["evidence"] = f"Wikidata P1451 (motto text): {motto}"
        ...
        for row in rows:
            ...
            if wiki and wiki.get("extract"):
                if not row.get("brief", {}).get("value"):
                    row["brief"] = leaf(wiki["extract"], wiki["page_url"], evidence=wiki["extract"])
                if wiki.get("photo_url") and "photo_url" in row:
                    row["photo_url"] = leaf(wiki["photo_url"], wiki["page_url"], evidence=f"Wikipedia thumbnail: {wiki['photo_url']}")
                filled += 1
            if "linkedin_url" in row and not row.get("linkedin_url", {}).get("value"):
                link = _linkedin_search_link(name, company_name)
                # search link, not a verified profile match — nothing to quote
                row["linkedin_url"] = leaf(link, link, evidence=None)
        if rows:
        ...
        try:
            ...
            if candidates:
                ...
                if info:
                    ...
                    row["revenue"] = leaf(info.get("totalRevenue"), src, evidence=f"totalRevenue: {info.get('totalRevenue')}")
                    row["employees"] = leaf(info.get("fullTimeEmployees"), src, evidence=f"fullTimeEmployees: {info.get('fullTimeEmployees')}")
                    row["market_cap"] = leaf(info.get("marketCap"), src, evidence=f"marketCap: {info.get('marketCap')}")
        except SystemExit:
...
def fill_deals(template, company_name):
    ...
    for r in results:
        ...
        # Each field's evidence must actually support THAT field — desc (e.g.
        # "LABOR", "AWARDED RECOMPETE.") never contains a vendor name or a
        # date, so reusing it as evidence for vendor/start_date/end_date
        # failed faithfulness scoring (0.00 across the board, confirmed:
        # judge correctly saw no vendor/date text in the evidence it was
        # given). Point each field at the actual USAspending API field it
        # came from instead — real provenance, not a shared unrelated string.
        rows.append({
            "vendor": leaf(recipient or q, url,
                            evidence=f"USAspending.gov Recipient Name: {recipient}" if recipient else None),
            "start_date": leaf(start, url,
                                evidence=f"USAspending.gov Start Date: {start}" if start else None),
            "end_date": leaf(end, url,
                              evidence=f"USAspending.gov End Date: {end}" if end else None),
            "contract_details": leaf(details, url, evidence=desc or None),
        })
    if rows:
